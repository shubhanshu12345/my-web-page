...
router.post('/register', async (req, res) => {
   ...
   const { name, email, address, password, cpassword } = req.body;//getting data by object destructuring
   
   if (!name || !email || !address || !password || !cpassword) { //user should fill all feild
      return res.status(422).json({ error: "plz fill all feild" })
   }
   ...
   try {
      ...
      if (userExist) {
         ...
         const user = new User({ name, email, address, password, cpassword })// adding data to database || if both key and value and are same no need to write twice 
...
router.post('/register2', async (req, res) => {
   ...
   const { name, email, address, oldemail } = req.body;
   ...
   try {
      var myquery = { email: oldemail };
      var newvalues = { $set: {name: name, address: address,email:email } };
